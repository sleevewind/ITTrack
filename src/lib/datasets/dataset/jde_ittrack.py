...
class MultiScaleJD(LoadImagesAndLabels):
    ...
    def gen_multi_scale_input_whs(self, num_scales=256, min_ratio=1, max_ratio=1):
        """
    ...
    def __getitem__(self, idx):
        ...
        pre_id = int(img_path.split('/')[-1].split(".jpg")[0])-random.randint(1,5)
        pre_id = max(1,pre_id)
        pre_image_id = "{0:07d}".format(pre_id)
        ...
        if not os.path.isfile(pre_img_path):
            ...
            pre_imgs, pre_labels, pre_img_path, (pre_input_h, pre_input_w) = self.get_data(pre_img_path, pre_label_path,
                                                                                           width, height)
        ...
        location_class = np.zeros((self.max_objs, 3), dtype=np.float32)
        ...
        for k in range(num_objs):
            ...
            if h > 0 and w > 0:
                ...
                location_class[k] = int(ct_int[0]),int(ct_int[1]),int(cls_id)
        ...
        # --- GT of detection
        pre_hm = np.zeros((self.num_classes, output_h, output_w), dtype=np.float32)
        pre_wh = np.zeros((self.max_objs, 2), dtype=np.float32)
        pre_reg = np.zeros((self.max_objs, 2), dtype=np.float32)
        pre_ind = np.zeros((self.max_objs,), dtype=np.int64)
        pre_reg_mask = np.zeros((self.max_objs,), dtype=np.uint8)
        pre_location_class = np.zeros((self.max_objs, 3), dtype=np.float32)

        if self.opt.id_weight > 0:
            # --- GT of ReID
            pre_ids = np.zeros((self.max_objs,), dtype=np.int64)

            pre_cls_tr_ids = np.zeros((self.num_classes, output_h, output_w), dtype=np.int64)

            pre_cls_id_map = np.full((1, output_h, output_w), -1, dtype=np.int64)  # 1×H×W

        # Gauss function definition
        pre_draw_gaussian = draw_msra_gaussian if self.opt.mse_loss else draw_umich_gaussian

        for k in range(pre_num_objs):
            ...
            if pre_h > 0 and pre_w > 0:
                ...
                # offset regression
                pre_reg[k] = pre_ct - pre_ct_int
                pre_reg_mask[k] = 1
                pre_location_class[k] = int(pre_ct_int[0]),int(pre_ct_int[1]),int(pre_cls_id)
                # --- GT of ReID
                if self.opt.id_weight > 0:
