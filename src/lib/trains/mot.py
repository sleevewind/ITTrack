...
class McMotLoss(torch.nn.Module):
    def __init__(self, opt):
        ...
        if opt.id_weight > 0:
            ...
            self.classifiers_pre = nn.ModuleDict()
            ...
            for cls_id, nID in self.nID_dict.items():
                ...
                self.classifiers_pre[str(cls_id)] = nn.Linear(self.emb_dim, nID)
            ...
            # self.s_id_pre = nn.Parameter(-1.05 * torch.ones(1))  # -1.05
            self.s_id = nn.Parameter(-1.05 * torch.ones(1))
        # scale factor of detection loss

        # self.s_det_pre = nn.Parameter(-1.85 * torch.ones(1))
        self.s_det = nn.Parameter(-1.85 * torch.ones(1))
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            if opt.id_weight > 0:  # if ReID is needed
                ...
                for cls_id, id_num in self.nID_dict.items():
                    ...
                    # pre_inds = torch.where(pre_cls_id_map == cls_id)
                    ...
                    pre_cls_id_pred = self.classifiers_pre[str(cls_id)].forward(pre_cls_id_head).contiguous()
                    ...
                    # pre_reid_loss += self.ce_loss(pre_cls_id_pred, pre_cls_id_target) / float(
                    #     pre_cls_id_target.nelement())
        ...
        if opt.id_weight > 0:
            loss = torch.exp(-self.s_det) * (det_loss + pre_det_loss)\
                   + torch.exp(-self.s_id) * reid_loss \
                   + (self.s_det + self.s_id)

            # pre_loss = torch.exp(-self.s_det_pre) * pre_det_loss \
            #            + torch.exp(-self.s_id_pre) * pre_reid_loss \
            #            + (self.s_det_pre + self.s_id_pre)
...
class MotTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        if opt.id_weight > 0:  # tracking loss including det loss and re-id loss
            loss_states = [
                           'loss',
                           'hm_loss', 'wh_loss', 'off_loss', 'id_loss',
                           'pre_hm_loss']
